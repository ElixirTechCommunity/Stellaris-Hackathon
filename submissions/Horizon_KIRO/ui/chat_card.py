...
# ── Graph visualization (optional — gracefully disabled if not installed) ──
try:
    from graph.jarvis_graph import run_viz
    from graph.server import broadcast_sync
    _VIZ_AVAILABLE = True
except Exception:
...
from PyQt5.QtGui import QFont, QColor, QTextCursor, QIcon, QKeyEvent
from PyQt5.QtCore import QSize

logger = logging.getLogger(__name__)


class CommandInputBox(QLineEdit):
    """Custom QLineEdit for command input with proper Enter key handling."""
    
    send_command = pyqtSignal()
    
    def keyPressEvent(self, event: QKeyEvent):
        """Handle key press events."""
        # Only send command on Enter/Return when typing in this box
...
class ChatCard(QDialog):
    ...
    def __init__(self, agent, voice_listener=None, parent=None):
        ...
        # Enable close button and window decorations
    ...
    def send_command(self):
        ...
        # ── Kick off live graph visualization (parallel daemon thread) ──
        self._fire_viz_graph(command)
    
    def _fire_viz_graph(self, command: str):
        """
        Run the lightweight LangGraph visualization in a background daemon thread.
        Node-activation events are broadcast via WebSocket to the browser dashboard.
        Never blocks the main GUI or the real agent thread.
        """
        ...
        def _run():
            try:
                # Notify browser: query started
                broadcast_sync({"type": "query_start", "query": command})

                # Build a node callback that forwards events to the browser
                def _node_cb(node_name: str):
                    broadcast_sync({"type": "node_active", "node": node_name})

                # Run the visualization graph
                final_state = run_viz(command, node_callback=_node_cb)

                # Notify browser: query done
                broadcast_sync({
                    "type":     "query_done",
                ...
                logger.warning(f"[viz] Graph run error: {exc}")

        t = threading.Thread(target=_run, daemon=True, name="JarvisVizRun")
        t.start()
    ...
    def closeEvent(self, event):
        """Handle chat card close - closes the entire JARVIS app"""
        try:
            # Stop voice thread if running
            if self.voice_thread and self.voice_thread.isRunning():
                self.voice_thread.quit()
                self.voice_thread.wait(2000)  # Wait up to 2 seconds
            
            # Stop execution thread if running
            if self.execution_thread and self.execution_thread.isRunning():
                self.execution_thread.quit()
                self.execution_thread.wait(2000)  # Wait up to 2 seconds
            
            # Restore stdout
            if self.stdout_redirector:
                sys.stdout = self.stdout_redirector.original_stdout
        except Exception as e:
            logger.warning(f"Error during closeEvent: {e}")
    ...
    def reject(self):
        """Handle dialog rejection - closes the entire JARVIS app"""
        try:
            # Stop voice thread if running
            if self.voice_thread and self.voice_thread.isRunning():
                self.voice_thread.quit()
            ...
            logger.warning(f"Error during reject: {e}")
    ...
    def keyPressEvent(self, event: QKeyEvent):
        """Handle keyboard events - Escape closes the dialog"""
