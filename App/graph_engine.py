...
class GraphExtractor:
    def __init__(self):
        ...
        print("🧠 Booting up Local GraphVision AI Models...")
        
       # Update these lines in your __init__
        classifier_path = resource_path("graph_classifier_real.pth")
        pie_model_path = resource_path("pie_regressor_stable.pth")
        yolo_path = resource_path("best2.pt")       
        dot_yolo_path = resource_path("best1.pt")
        
        # Ensure files exist before trying to load them
        for path in [classifier_path, pie_model_path, yolo_path, dot_yolo_path]:
            if not os.path.exists(path):
                raise FileNotFoundError(f"Missing model file: {path}. Please place it in the script directory.")
